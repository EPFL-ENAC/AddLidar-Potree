...
export class Viewer extends EventDispatcher {
  constructor(domElement, args = {}) {
    ...
    try {
      ...
      this.filterPointSourceIDSubset = new Int32Array(1024).fill(1);
  ...
  setFilterPointSourceIDSubset(pointSourceIDs) {
    ...
    this.filterPointSourceIDSubset = subset;

    for (let pointcloud of this.scene.pointclouds) {
      if (pointcloud.material) {
        pointcloud.material.setFilterPointSourceIDSubset(pointSourceIDs);
      }
    }

    this.dispatchEvent({
      type: "filter_point_source_id_subset_changed",
      viewer: this,
    });
  }

  clearFilterPointSourceIDSubset() {
    // This will include all IDs by setting each to 1
    for (let pointcloud of this.scene.pointclouds) {
      if (pointcloud.material) {
        pointcloud.material.clearFilterPointSourceIDSubset();
      }
    }
  ...
  update(delta, timestamp) {
    ...
    for (let pointcloud of visiblePointClouds) {
      ...
      material.uniforms.uFilterPointSourceIDSubset.value =
        this.filterPointSourceIDSubset;
