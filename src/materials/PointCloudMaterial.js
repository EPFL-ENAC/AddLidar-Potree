...
export class PointCloudMaterial extends THREE.RawShaderMaterial {
  constructor(parameters = {}) {
    ...
    this.uniforms = {
      ...
      uFilterPointSourceIDSubset: {
        type: "iv",
        value: new Int32Array(1024).fill(1),
      }, // Default: include all IDs
  ...
  updateShaderSource() {
    ...
    console.log("DEFINES", definesString);
  ...
  // Methods for filtering point source IDs by subset
  setFilterPointSourceIDSubset(pointSourceIDs) {
    // If no array or empty array provided, include all IDs (set all to 1)
    if (!pointSourceIDs || pointSourceIDs.length === 0) {
      this.uniforms.uFilterPointSourceIDSubset.value.fill(1);
      this.removeDefine("clip_point_source_id_subset_enabled");
      return;
    }

    // Start by excluding all IDs (set all to 0)
    const subset = new Int32Array(1024).fill(0);

    // Set the specified IDs to 1 (included)
    for (let id of pointSourceIDs) {
      if (id >= 0 && id < 1024) {
        subset[id] = 1;
      }
    }

    this.uniforms.uFilterPointSourceIDSubset.value = subset;
    this.setDefine(
      "clip_point_source_id_subset_enabled",
      "#define clip_point_source_id_subset_enabled"
    );
  ...
  clearFilterPointSourceIDSubset() {
    // Include all point source IDs
    this.uniforms.uFilterPointSourceIDSubset.value.fill(1);
    this.removeDefine("clip_point_source_id_subset_enabled");
